...
class EditEmergencyPage extends StatefulWidget {
  ...
  const EditEmergencyPage({super.key, required this.id, required this.data});
...
class _EditEmergencyPageState extends State<EditEmergencyPage> {
  ...
  // same presets as AddEmergencyPage
  ...
  int _selectedPreset = 6; // default → General
  ...
  void initState() {
    ...
    // Try to match existing colour to a preset
    final existingColor = (widget.data['color'] as String? ?? '').toUpperCase();
    for (int i = 0; i < _presets.length; i++) {
      if (_presets[i].color.toUpperCase() == existingColor) {
        _selectedPreset = i;
        break;
      }
    }
