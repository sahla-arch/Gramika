...
class EditAdsPage extends StatefulWidget {
  ...
  const EditAdsPage({super.key, required this.adsId, required this.ads});
...
class _EditAdsPageState extends State<EditAdsPage> {
  ...
  Future<void> pickImage() async {
    ...
    setState(() {
      selectedImage = bytes;
      existingImgUrl = ''; // clear old so new image takes over
    });
  }
  ...
  bool _validate() {
    ...
    if (selectedImage == null && existingImgUrl.isEmpty) {
  ...
  Future<void> saveAds() async {
    ...
    // Resolve final image — new device pick wins, else keep existing
    ...
    await FirebaseFirestore.instance
        ...
        .update({
          ...
          'createdBy': widget.ads['createdBy'] ?? 'Super Admin',
          'createdAt': widget.ads['createdAt'] ?? Timestamp.now(),
          'youtubeLinks': widget.ads['youtubeLinks'] ?? [],
        });

    if (mounted) {
      _showSnack('Ad updated successfully');
      Navigator.pop(context, true); // true → caller refreshes list
    }
  }
                ...
                children: [
                  ...
                  Center(
                    child: TextButton.icon(
                      ...
                      label: Text(
                        (selectedImage != null || existingImgUrl.isNotEmpty)
                            ? 'Replace Image'
                            : 'Upload from Device',
  ...
  Widget _buildImagePreview() {
    ...
    // Existing value from Firestore
    if (existingImgUrl.isNotEmpty) {
      // Cloudinary / HTTP URL
